...
fixed = ''.join(output)
with open('d:\\KinSU\\kinsu-site\\index.html', 'w', encoding='ascii') as f:
    f.write(fixed)

print(f'ASCII file size: {len(fixed)} bytes')
print(f'Max byte value: {max(fixed.encode("ascii"))}')
non_ascii = [c for c in fixed if ord(c) > 127]
