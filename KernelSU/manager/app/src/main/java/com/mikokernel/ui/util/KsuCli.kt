...
private fun getKsuDaemonPath(): String {
    return ksuApp.applicationInfo.nativeLibraryDir + File.separator + "libfollkerneld.so"
}
...
fun flashModule(
    ...
    with(resolver.openInputStream(uri)) {
        ...
        Log.i("FollKernel", "install module $uri result: $result")
...
fun runModuleAction(
    ...
    Log.i("FollKernel", "Module runAction result: $result")
...
fun installBoot(
    ...
    when (lkm) {
        ...
        LkmSelection.KmiNone -> {
            ...
            if (kmi != null) {
                ...
                lkmFile = try {
                    val file = File(ksuApp.cacheDir, "kernelsu-auto-${kmi}.ko")
                    ksuApp.assets.open(assetName).use { input ->
                        file.outputStream().use { output -> input.copyTo(output) }
                    }
                    file
    ...
    val result = flashWithIO("${getKsuDaemonPath()} $cmd", onStdout, onStderr)
    Log.i("FollKernel", "install boot result: ${result.isSuccess}")
